...
local current_text = EntityGetVariableString( current_target, "evaisa.arena_dps_tracker_text", "" );
if current_target ~= 0 and last_text ~= current_text then
    last_text = current_text;
    local height = 20;
    local sprite = EntityGetFirstComponent( entity, "SpriteComponent", "evaisa.arena_dps_tracker" );
    if sprite then
