...
local current_text = EntityGetVariableString( current_target, "evaisa.arena_custom_damage_numbers_text", "" );
if current_target ~= 0 and last_text ~= current_text then
    last_text = current_text;
    local height = 20;
    local sprite = EntityGetFirstComponent( entity, "SpriteComponent", "evaisa.arena_custom_damage_number" );
    if sprite then
